...
class Provider extends events.EventEmitter {
  ...
  useTenantIssuer = false;

  get issuer() {
    console.trace('provider issuer');
    return this.baseIssuer;
  }

  getIssuerForTenant(client, source) {
    let issuer = this.baseIssuer;
    if (client) {
      const url = new URL(this.baseIssuer);
      issuer = `${url.protocol}//${url.host}/${client.tenantId}/v1.0`;
    }
    // console.info('>getIssuerForTenant', source, issuer);
    return issuer;
  }
  ...
  constructor(issuer, setup) {
    ...
    this.baseIssuer = issuer;

    const configuration = new Configuration(setup);

    this.useTenantIssuer = configuration.features.useTenantIssuer?.enabled || false;
