...
export default (provider) => {
  ...
  function getOutAndEmit(ctx, err, state) {
    ...
    const issuer = ctx.oidc.provider.useTenantIssuer ? ctx.oidc.provider.getIssuerForTenant(client, 'jwt') : ctx.oidc.provider.issuer;
    provider.getIssuerForTenant(client, 'authorization_error_handler');
    const out = { ...errOut(err, state), iss: issuer };
    console.log(`Error@oidc-provider::2`, err);
